...
def getOssecConfig(initconf, path):
    if os.path.isfile(path):
        ...
        if initconf["NAME"] != "Khulnasoft" or not os.path.exists(initconf["DIRECTORY"]):
            print "Seems like there is no correct Khulnasoft installation "
            sys.exit(1)
    else:
        print "Seems like there is no Khulnasoft installation or ossec-init.conf is missing."
        sys.exit(1)
...
def restart_analysisd():
    print "Restarting khulnasoft-manager..."
    ret = os.system('systemctl restart khulnasoft-manager')
...
class OssecTester(object):
    ...
    def buildCmd(self, rule, alert, decoder):
        cmd = ['%s/khulnasoft-logtest' % (self._ossec_path), ]
        cmd += ['-q']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='This script tests Khulnasoft rules.')
    ...
    parser.add_argument('--custom-ruleset', '-c', action='store_true', dest='custom',
                        help='Use -c or --custom-ruleset to test custom rules and decoders. WARNING: This will cause khulnasoft-manager restart')
